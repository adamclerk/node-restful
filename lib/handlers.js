...
exports.last = function (req, res, next) {
  if (res.locals.bundle) {
    if (req.body.format === 'js') {
      ...
      return res.set('etag', res.locals.etag).status(res.locals.status_code).json(res.locals.bundle);
    }
  }
  res.send();
};
...
exports.put = function (req, res, next) {
  ...
  if (!_.isUndefined(id) && !_.isUndefined(req.headers['if-match'])) {
    req.quer.find({_id: id}, function (err, oldObj) {
      var currentEtag = md5(JSON.stringify(oldObj[0]));
      var requestedEtag = req.headers['if-match'];
      if (requestedEtag !== currentEtag) {
        exports.respond(res, 412, 'Precondition Failed');
        next();
        return;
      }
    })
...
exports.objectNotFound = function () {
  return {
    ...
    errors: {
      _id: {
        message: 'Could not find object with specified attribute'
      }
    }
  };
};
...
exports.respond = function (res, statusCode, content) {
  ...
  res.locals.etag = md5(JSON.stringify(content));
};
